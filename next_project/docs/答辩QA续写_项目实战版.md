...
4. C++ 动态回放侧在执行推进前做连续胶囊体检查，发现下一小段不安全就阻止前进，并记录 `clearance_blocked`。
...
| 路径接收 | `_accept_online_path` 连续 SDF clearance 门禁 | 规划网格膨胀与执行前胶囊体检查 |
| 执行兜底 | 不安全新路径不覆盖 `local_path` | `safe_advance_along_path` 阻止危险推进 |
| 日志 | `replan_events` | `clearance_blocked` 写入 replay frame |

所以不是逐行完全一样，而是在“新路径不能未经连续安全检查就执行”这个安全语义上对齐。

### Q10：C++ 里的“胶囊体检查”是什么意思？
...
### Q12：如果老师问“Python 侧为什么不是也叫胶囊体检查”怎么答？

**答：**

Python 侧做的是路径级连续 SDF clearance 检查，本质上也是在验证线段附近是否满足安全距离，只是表达为“沿线段采样并查询 SDF”。C++ 动态回放侧是在执行推进时用“胶囊体”这个几何概念描述同一类安全约束。
...
### Q16：如果演示中出现 `clearance_blocked`，是不是说明算法失败？
...
4. C++ 胶囊体执行：`next_project/cpp/include/dynamic_scenario.hpp::capsule_clear`
5. C++ 安全推进：`safe_advance_along_path`
6. C++ 回放字段：`next_project/cpp/src/dynamic_main.cpp` 输出 `clearance_blocked`
7. Web 显示：`next_project/web/dynamic_replay.html`

---

## 6. 可靠性与测试

### Q19：如何证明这次 Python 门禁不是只写了代码、没有验证？
...
### Q25：如果老师问“你为什么现在才补 Python 门禁”，怎么答？

**答：**

原来 Python 侧已经有 SDF-aware grid、FIRI、fallback 和线段安全检查，但在线新路径接收处还缺少一个统一验收口。也就是说，安全机制分散存在，但最后接收路径时没有强制要求“FIRI 和 fallback 都失败就拒绝”。
...
Python 侧重点是新路径接收前的连续 SDF clearance 验收，C++ 侧重点是执行推进前的胶囊体安全检查，两者对齐的是“不安全路径不能进入执行主线”这个语义。
...
2. 再说机制：Python 接收门禁或 C++ 胶囊体检查阻止不安全路径。
