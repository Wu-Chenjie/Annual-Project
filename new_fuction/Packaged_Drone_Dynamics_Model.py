...
class Drone:
    ...
    def _resolve_wind(self, wind):
        if wind is None:
            return self.wind
        if callable(wind):
            return np.array(wind(self.dt), dtype=float)
    ...
    def dynamics(self, t, state, u, wind=None):
        ...
        #把风速分解为相对于地面和相对于无人机的风速，计算空气阻力，并将其加入加速度计算中。
        wind_vec = self._resolve_wind(wind)
        ...
        acc = 1/self.m * (R @ np.array([0,0,u[0]]) + drag - np.array([0,0,self.m * self.g]))
        omega = np.array([wx, wy, wz])
        tau = np.array([u[1], u[2], u[3]])
        omega_dot = self.I_inv @ (tau - np.cross(omega, self.I @ omega))
        #把机体系的角速度[wx, wy, wz]转换为欧拉角的变化率[roll_dot, pitch_dot, yaw_dot]
