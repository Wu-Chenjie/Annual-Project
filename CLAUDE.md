...
仓库由「主活动子项目 next_project」、「历史代码归档 old_code」、「论文与素材」与若干顶层文档构成。next_project 进一步分为 Python 主仿真（core 内核 + simulations 编排）与 `cpp/` 可运行子集，两者输出口径一致，便于交叉验证。

- 入口层：`next_project/main.py`、`next_project/cpp/src/main.cpp`、`next_project/cpp/src/warehouse_main.cpp`、`next_project/cpp/src/dynamic_main.cpp`
- 编排层：`next_project/simulations/`（仿真流程、批量评测、障碍场景、可视化）
- 模型层：`next_project/core/`（动力学、控制、滑模、风场、拓扑、旋翼、分配、障碍物、传感器、地图加载）
- 规划层：`next_project/core/planning/`（A*、Hybrid A*、Dijkstra、D* Lite、RRT*、Informed RRT*、ESDF-like 软代价、GNN 可见图、双模式调度、在线重规划器）
...
| `next_project/core/planning/` | 路径规划算法集：A*、Hybrid A*、Dijkstra、D* Lite、RRT*、Informed RRT*、ESDF-like 软代价、GNN 可见图、双模式调度、在线重规划器 | Python | `base.py` |
| `next_project/simulations/` | 仿真编排：编队仿真、障碍场景仿真、批量评测、可视化 | Python | `formation_simulation.py` |
| `next_project/cpp/` | next_project 的 C++ 可运行子集，含路径规划、避障、dynamic replay 和报告输出，性能优先 | C++20 | `src/main.cpp` |
...
| `warehouse_a` | 仓库场景 A\* 版（在线 + GNN Danger 模式 + ESDF-like 软代价） |
...
./build/sim_dynamic_replay.exe input.json -o output.json
```

## 测试策略

当前主仿真采用「指标阈值法」自检：`main.py` 在仿真结束后比对从机平均/最大误差是否 < 0.3 m 阈值；`simulations/benchmark.py` 通过多随机种子复现实验输出 mean/std/worst 指标。`tests/` 目录包含 Web 输入防线、C++ 静态同步、schema、GNN、障碍场景、报告和跨线回归测试。GitHub Actions 默认运行 `python -m pytest -m "not slow"` 与 C++ CMake 全目标构建。

## 编码规范

- Python 使用 `from __future__ import annotations` + 类型注解 + `dataclass` 配置对象。
- 中文文档字符串说明「用途/原理」，便于跨学科同事阅读。
- C++ 重构使用 C++20，`-O3`/MSVC Release 优化，固定大小数组减少动态分配；MSVC 构建通过 CMake 使用 `/utf-8`。
...
- 修改控制/动力学公式前，先阅读 `next_project/docs/技术文档.md` 与相关测试。
...
- 2026-04-30：**GNN 双模式 + APF 增强 + 容错拓扑重构综合落地**（基于 4 篇论文，修复 7 项冲突修补 C1-C7）：
  ...
  - 文档更新：`技术文档.md`、`capability-matrix.md`、`中期验收记录.md`
- 2026-04-29：更新项目结构，反映已完成的核心变更：
- 新增 `core/planning/` 路径规划子包（A*、Hybrid A*、Dijkstra、D* Lite、RRT*、Informed RRT*、ESDF-like 软代价、在线重规划器）
  ...
  - 新增 `tests/` 测试目录
- C++ 端大幅扩展：新增避障、路径规划（A*、Hybrid A*、D* Lite、ESDF-like 栅格距离近似）、传感器、占据栅格、人工势场、在线重规划器等模块
