...
最新完整 ROS 回归：**114 tests, 0 errors, 0 failures, 0 skipped**。新增首次观测立即细分的跨机状态测试，确保没有旧父区域记录时仍传播 splitR。候选筛选的向量化在实测三维地图上与原实现的区域、状态、视点及排序完全一致（主机测量约 0.80 s → 0.04 s）。
...
- native-ninth：修复评估几何口径后的统一版本，真实 Gazebo + RViz，同屏 1920×1080，24 倍录屏时钟播放。
- 专用 Colima profile annual-fusion，6 CPU / 8 GB，Docker context colima-annual-fusion，容器 annual-fusion-run。
- 实时目录 /tmp/annual-fusion-native-ninth；录制 exec session 10909，超时 14400 s。
