...
双机窗口目标为 `J = T_a + T_b + 0.25 × max(T_a, T_b)`，其中 T 是沿可通行路线计算的预计运动时间。区域负载为 `1 + 未知体素数 × 分辨率³`，并计入窗口外固定负载；这是探索工作量约束，不是电池续航模型。观测位姿则按未知体积收益除以平移、转向及驻留时间评分，再加入第二视点的增量收益与下一任务出口代价。
