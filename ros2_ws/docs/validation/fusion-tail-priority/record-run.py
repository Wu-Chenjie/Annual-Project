...
def main():
    ...
    env=dict(os.environ,OPENBLAS_NUM_THREADS='1',OMP_NUM_THREADS='1',LP_NUM_THREADS='2',DISPLAY=':99',LIBGL_ALWAYS_SOFTWARE='1',QT_X11_NO_MITSHM='1',PYTHONDONTWRITEBYTECODE='1',
        GZ_PARTITION='annual_tail_'+uuid.uuid4().hex,ROS_DOMAIN_ID=str(20+int(uuid.uuid4().hex[:4],16)%180))
    ...
    (out/'run-configuration.json').write_text(json.dumps(dict(base_commit=metadata['base_commit'],includes_uncommitted_changes=True,arguments=vars(args),comparison='Same 3D scene, sensors, estimation, physical limits and fault-trigger rules as optimized-final. Frozen current priority changes; primary metric is t95 minus t90.'),indent=2))
    servers=[]
    ...
    try:
        ...
        log.close()
    final=out/'tail-priority-exploration.mp4'
    title=f'FUSED 3D EXPLORATION + UNKNOWN-REGION PRIORITY | {args.speed:g}x recording speed'
