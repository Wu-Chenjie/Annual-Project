...
def main():
    ...
    metadata=json.loads(Path(args.manifest).read_text());manifest=metadata['files']
    mismatches=[n for n,h in manifest.items() if hashlib.sha256((root/n).read_bytes()).hexdigest()!=h]
    ...
    env=dict(os.environ,OPENBLAS_NUM_THREADS='1',OMP_NUM_THREADS='1',LP_NUM_THREADS='2',DISPLAY=':99',LIBGL_ALWAYS_SOFTWARE='1',QT_X11_NO_MITSHM='1',PYTHONDONTWRITEBYTECODE='1',
        GZ_PARTITION='annual_integrated_'+uuid.uuid4().hex,ROS_DOMAIN_ID=str(20+int(uuid.uuid4().hex[:4],16)%180))
    ...
    (out/'run-configuration.json').write_text(json.dumps(dict(base_commit=metadata['base_commit'],includes_uncommitted_changes=True,arguments=vars(args),comparison='Same 3D scene, sensors, estimation, physical limits and fault-trigger rules as optimized-final. Frozen integrated route/information changes; primary metric is t95, with t95 minus t90 reported alongside.'),indent=2))
    servers=[]
    ...
    try:
        ...
        proc=subprocess.Popen(['ros2','launch','annual_swarm','decentralized_search.launch.py','headless:=false','rviz:=true','visualize:=true',f'output_dir:={out}'],env=env,stdout=log,stderr=log,start_new_session=True)
        ...
        while time.monotonic()-start<args.wall_limit:
            ...
            if file.exists():
                ...
                if s['simulation_time']>50 and len(s.get('agent_ages',{}))!=3:raise RuntimeError('An exploration agent failed to start')
                if max(s.get('agent_ages',{}).values(),default=0)>15.:raise RuntimeError('Exploration heartbeat stopped')
        ...
        if not (out/'observed_final.npz').exists() and proc and proc.poll() is None:
            try:
                subprocess.run([sys.executable,str(Path(__file__).with_name('snapshot_map.py')),str(out)],env=env,timeout=12,check=True,stdout=log,stderr=log)
            except Exception as exc:
                (out/'map-snapshot-error.txt').write_text(repr(exc))
        ...
        log.close()
    final=out/'integrated-exploration.mp4'
    title=f'FUSED 3D EXPLORATION | JOINT ROUTE + INFORMATION | {args.speed:g}x recording speed'
