...
def measure(root):
    ...
    for phase,(begin,finish) in phases.items():
        ...
        aging_shares=[((p['aging_multiplier']-1)/p['aging_multiplier'] if 'aging_multiplier' in p else min(1.,aging_rate*p['wait_s']/p['score'])) for p in priorities if aging_rate is not None and p.get('score',0)>0]
...
def figure(result,roots,dest):
    ...
    labels=['Previous optimized fusion','Integrated route + information']; colors=['#77889b','#008b8b']
